...
from bot.helper.ext_utils.bot_utils import get_readable_time, is_gdrive_link, is_gdtot_link,is_hubdrive_link, is_appdrive_link, get_readable_time, new_thread
from bot.helper.mirror_utils.download_utils.direct_link_generator import gdtot, hubdrive, appdrive
...
def _clone(message, bot, multi=0):
    ...
    is_gdtot, is_hubdrive, is_appdrive = is_gdtot_link(link), is_hubdrive_link(link), is_appdrive_link(link)
    if any([is_gdtot, is_hubdrive, is_appdrive]):
    ...
    if is_gdrive_link(link):
        ...
        sendMessage('Send Gdrive or GDToT or Appdrive or HubDrive link along with command or by replying to the link by command', bot, message)
