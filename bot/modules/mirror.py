...
from bot import bot, Interval, INDEX_URL, VIEW_LINK, aria2, QB_SEED, dispatcher, DOWNLOAD_DIR, \
...
class MirrorListener:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name: str):
        if not self.isPrivate and INCOMPLETE_TASK_NOTIFIER and DB_URI is not None:
            DbManger().rm_complete_task(self.message.link)
        msg = f"<b>Nᴀᴍᴇ: </b><code>{escape(name)}</code>\n\n<b>Sɪᴢᴇ: </b>{size}"
        if self.isLeech:
            msg += f'\n<b>Tᴏᴛᴀʟ Fɪʟᴇs: </b>{folders}'
            if typ != 0:
                msg += f'\n<b>Cᴏʀʀᴜᴘᴛᴇᴅ Fɪʟᴇs: </b>{typ}'
            msg += f"\n\n<b>Eʟᴀᴘsᴇᴅ:</b> {get_readable_time(time() - self.elapsed_time)}"
            msg += f'\n<b>#Lᴇᴇᴄʜᴇᴅ Fᴏʀ: </b>{self.tag}\n\n'
            pmsg = 'I Hᴀᴠᴇ Sᴇɴᴛ Yᴏᴜʀ Fɪʟᴇs Iɴ PM'
            sendPrivateMsg(msg, self.bot, self.message)
            sendMessage(msg + pmsg, self.bot, self.message)
        else:
            msg += f'\n\n<b>Tʏᴘᴇ: </b>{typ}'
            if ospath.isdir(f'{DOWNLOAD_DIR}{self.uid}/{name}'):
                msg += f'\n<b>SᴜʙFᴏʟᴅᴇʀs: </b>{folders}'
                msg += f'\n<b>Fɪʟᴇs: </b>{files}'
            msg += f"\n\n<b>Eʟᴀᴘsᴇᴅ:</b> {get_readable_time(time() - self.elapsed_time)}"
            msg += f'\n\n<b>Rᴇǫ Bʏ: </b>{self.tag}'
...
def _mirror(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, multi=0):
    ...
    try:
        ...
        buttons.buildbutton("🤩 Sᴛᴀʀᴛ Bᴏᴛ 🤩", f"{botstart}")
        startwarn = f"🙋‍♂️Hᴇʏ! {uname},\n\n<b>🤭Yᴏᴜ ɴᴇᴇᴅ ᴛᴏ Sᴛᴀʀᴛ Bᴏᴛ ᴜꜱɪɴɢ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ.</b>\n\n" \
                    f"Iᴛꜱ ɴᴇᴇᴅᴇᴅ ꜱᴏ ʙᴏᴛ ᴄᴀɴ ꜱᴇɴᴅ ʏᴏᴜʀ ᴍɪʀʀᴏʀ/ᴄʟᴏɴᴇ/ʟᴇᴇᴄʜᴇᴅ ꜰɪʟᴇꜱ ɪɴ ᴘᴍ."
        sendMarkup(startwarn, bot, message, InlineKeyboardMarkup(buttons.build_menu(1)))
        return
